...
static const char *trapname(int trapno)
{
	static const char * const excnames[] = {
		...
		[IRQ_OFFSET+IRQ_TIMER]    = "Timer Interrupt",
		[IRQ_OFFSET+IRQ_KBD]      = "KBD Interrupt",
		[IRQ_OFFSET+IRQ_SERIAL]   = "Serial Interrupt",
		[IRQ_OFFSET+IRQ_SPURIOUS] = "Spurious Interrupt",
		[IRQ_OFFSET+IRQ_IDE]      = "IDE Interrupt",
...
trap_init_percpu(void)
{
	...
	// Calculate the TSS selector for this cpu
	int tsso = GD_TSS0+(cpunum() << 3);

	// Initialize the TSS slot of the gdt.
	gdt[tsso >> 3] = SEG16(STS_T32A, (uint32_t) (&thiscpu->cpu_ts),
